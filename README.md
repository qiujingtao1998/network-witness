...
+ `comparison.py` is used to generate Fig. S3 in the Supplementary Information.
  - Here, we consider the qubit case, and two different global noises are considered.
    + The white noise 
    $$ \rho = (1-\lambda)\bigotimes_{e\in E} \Phi_e + \lambda \bigotimes_{e\in E} \frac{\mathbb{I}_e}{4}. $$
    + The dephasing noise
    $$ \rho = (1-\lambda)\bigotimes_{e\in E} \Phi_e + \lambda \Delta(\bigotimes_{e\in E} \Phi_e), $$
    where $\Delta$ denotes the operation that retains only the diagonal elements.